...
import { useState } from 'react';
import { api } from '../api/api'; // ✅ FIXED (mock → real api)
import TestTable from '../components/TestTable';
import { FlaskConical, Sparkles, AlertCircle, ShieldAlert, FileText } from 'lucide-react';
...
export default function GenerateTests() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ NORMALIZE TESTS (IMPORTANT 🔥)
      const formattedTests = data.tests.map((t, i) => ({
        id: i,
        title: t.test_name || `Test ${i + 1}`,
        steps: t.steps?.map(s => `${s.action} ${s.locator || ''}`) || [],
        expected: "Should work as expected",
        category: "Auth",
        priority: "medium"
      }));

      // ✅ SAVE FOR EXECUTION PAGE
  ...
  const riskColor =
    result?.risk_score >= 70 ? 'warn' :
    result?.risk_score >= 40 ? 'gold' :
    'neon';

  const riskLabel =
    result?.risk_score >= 70 ? 'HIGH' :
    ...
    <div className="page-enter space-y-6">
            ...
            {result && (
              <button
                ...
                onClick={() => {
                  ...
                  localStorage.removeItem("tests"); // cleanup
                }}
              >
                Clear
              </button>
      ...
      {loading && (
        <div className="glass-card p-8 text-center">
          <p className="text-text-muted">Generating test cases...</p>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="space-y-4 page-enter">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className={`glass-card px-4 py-2`}>
              <span className={`text-sm font-mono text-${riskColor}`}>
                Risk: {riskLabel} ({result.risk_score})
              </span>
            </div>
          </div>
